...
class kmeans(object):
	...
	def run(self, nmax = 100, eps = 1e-7):
		...
		for iter in range(0,nmax):
			...
			print("Iteration ", iter, " completed in ", iter_end_time - iter_st_time, " seconds.")
		end_time = time.time()
		print("Total Time taken for clustering ->>>> ", end_time-st_time, " seconds.")
...
if __name__ == '__main__':
	...
	res = []
	file_path = "input/c5_s100_f3.csv"
	...
	with timer.Timer(verbose=True):
		k1.run()
	#
	wrtr = csv.writer(open("./output/result.csv","w"))
	for x in k1.getOutput():
		wrtr.writerow(x)
	eWrtr = csv.writer(open("./output/error.csv","w"))
